...
public:
    ...
    void OnDraw(void) {
        ...
        }else if (CustomloadMenu.isActive()) {
            CustomloadMenu.render();
        ...
        else if(selectdifficultymenu.isActive()){
            selectdifficultymenu.render();
    ...
    void OnKeyboardDown(unsigned char key, int x_t, int y_t) {
        if (principalMenu.isActive()) {
            ...
            if (key == 13) { // ENTER
                int option = (int)CustomloadMenu.getSelectedOption();
                ...
                delete scene;
                scene = new BoardGL(variant,mode,difficulty);
                scene->init();
                scene->game->loadSavedGame(option);
                CustomloadMenu.setActive(false);
            }
            CustomloadMenu.handleInput(key);
            glutPostRedisplay();
        }else if (exitMenu.isActive()) {
            ...
            if (key == 13) { // ENTER
                ...
                switch (option) {
                    case EndGameMenu::Option::PVP:
                         delete scene;
                        scene = new BoardGL(variant,2,difficulty);
                        scene->init();
                        botMenu.setActive(false);
                        break;
                    case EndGameMenu::Option::BOT:
                        // Volver al menú principal
                        selectdifficultymenu.setActive(true);
                        botMenu.setActive(false);
                        break;
                }
            }
            ...
            variant = 0;  
            if (key == 13) { // ENTER
                SelectDifficultyMenu::Option option = selectdifficultymenu.getSelectedOption();
                 
                difficulty = (int)option - 16;

                colorMenu.setActive(true);
                selectdifficultymenu.setActive(false);
               
            }
            selectdifficultymenu.handleInput(key);
            glutPostRedisplay();
            ...
            if (key == 13) { // ENTER
                ...
                switch (option) {
                    case EndGameMenu::Option::RESTART:
                        // Reiniciar juego
                        delete scene;
                        scene = new BoardGL(variant,mode,difficulty);//pantalla de reinicio de juego para la seleccion de juego
                        scene->init();
    ...
    void OnTimer(int value) {
        if (!principalMenu.isActive() && !selectdifficultymenu.isActive() && !CustomloadMenu.isActive() && !modeSelectionMenu.isActive() && !inGameMenu.isActive() && !botMenu.isActive() && !colorMenu.isActive() && !endGameMenu.isActive()) {
